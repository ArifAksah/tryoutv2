...
export async function DashboardView({ user }: DashboardProps) {
    const admin = await isAdminUser();
    const { sections } = await fetchExamStructure();

    let tryoutPackages: TryoutPackageInfo[] = [];
    let userSubscriptions: { status: string; plan: { title: string } | null }[] = [];
    ...
    if (hasSupabasePublicEnv()) {
        ...
        const { data: subsData } = await supabase
            ...
            .select("status, plan:subscription_plans(title)")
            .eq("user_id", user?.id)
            .in("status", ["waiting", "approved"]);

        userSubscriptions = (subsData ?? []) as any[];
    ...
    return (
        <SidebarShell
            ...
            nav={[
                ...
                { href: "/pricing", label: "Langganan", description: "Paket & Harga", group: "Menu Utama" },
                ...
                {userSubscriptions.length > 0 && (
                    <section className="space-y-3">
                        {userSubscriptions.map((sub, idx) => (
                            <div key={idx} className={`rounded-xl border px-4 py-3 shadow-sm ${sub.status === "waiting"
                                    ? "border-amber-200 bg-amber-50 text-amber-800"
                                    : "border-emerald-200 bg-emerald-50 text-emerald-800"
                                }`}>
                                <div className="flex items-center gap-3">
                                    <div className={`flex h-8 w-8 items-center justify-center rounded-full bg-white shadow-sm ${sub.status === "waiting" ? "text-amber-500" : "text-emerald-500"
                                        }`}>
                                        {sub.status === "waiting" ? "⏳" : "✓"}
                                    </div>
                                    <div>
                                        <p className="text-sm font-bold">
                                            {sub.status === "waiting" ? "Menunggu Persetujuan" : "Langganan Aktif"}
                                        </p>
                                        <p className="text-xs opacity-90">
                                            Paket: <span className="font-semibold">{sub.plan?.title}</span>
                                            {sub.status === "waiting" ? " — Admin sedang memverifikasi permintaanmu." : " — Selamat belajar!"}
                                        </p>
                                    </div>
                                </div>
                            </div>
